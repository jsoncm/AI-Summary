...
def set_openai_proxy(proxy: str):
    st.session_state["OPENAI_PROXY"] = proxy
...
def sidebar():
    with st.sidebar:
        ...
        proxy = st.text_input(label="🔑 Proxy",
                                  placeholder="Enter your Proxy url",
                                  type="default",
                                  help="You can get Proxy from https://beta.openai.com/account/api-keys",
                                  value=_set_config(config_file, "PROXY", ""))
